...
library(readxl)
setwd("/home/bas/")
Amadeusdata <- read_excel("~/Downloads/Amadeus_Export_1.xlsx", sheet = "Results")
...
#Because there are a lot of duplicates, I want to filter the dataset to unique observations
...
library(rvest)
library(dplyr)
source <- read_html("https://en.wikipedia.org/wiki/List_of_members_of_the_19th_Bundestag")
...
dist.name <- stringdistmatrix(source1.devices$name, source2.devices$name, method = "osa") 
...
We have 120 politician-board members! That means that `r 120/708` = 17% of the Bundestag members are member of a Board of a publicly listed company, which is a quite significant amount! On the other hand, only a very small percentage, `r 120/73125` < 0.1% of the Board members is a politician, which is to be expected. 
