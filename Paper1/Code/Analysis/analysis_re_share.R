...
p3 <- results %>%
    filter(term == "xshare_Re") %>%
    ggplot(aes(x = law, y = estimate, 
               ymin = estimate - 1.65* std.error,
               ymax = estimate + 1.65* std.error)) + geom_pointrange() +
    coord_flip() + labs(title = "Effect of Real estate share on propensity to vote in favor of Laws", 
...
ggsave("./Figures/realestate_effectsizes_extensivemodel.png", p3, width = 25, units = "cm")
