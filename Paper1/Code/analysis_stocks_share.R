...
p4 <- results %>%
    filter(term == "share_shares") %>%
    ggplot(aes(x = law, y = estimate, 
               ymin = estimate - 1.65* std.error,
               ymax = estimate + 1.65* std.error)) + geom_pointrange() +
    coord_flip() + labs(title = "Effect of Stock share in portfolio on propensity to vote in favor of Laws", 
...
ggsave("./Figures/share_stocks_effectsizes_extensivemodel.png", p4, width = 25, units = "cm")

## Combine plots into one

#library(gridExtra)
p5 <- gridExtra::grid.arrange(p3, p4, nrow = 1)
ggsave("./Figures/re_share_effectsizes_combined.png", p5, width = 40, height = 23, units = "cm")
